...
A full-stack enterprise application for managing organizations, projects, tasks, users, analytics, and audit activity with strict multi-tenant isolation and role-based access control.

## Overview

This platform is designed as a secure multi-tenant SaaS-style application.

Each organization operates in an isolated workspace, while users receive different permissions based on their role.
...
The application is fully containerized using Docker Compose with separate containers for the frontend, backend, and PostgreSQL database.

![Enterprise Multi-Tenant Asset & Task Management Platform Architecture](./docs/architecture.png)
...
Tenant-aware authorization is applied throughout the backend to prevent users from accessing resources belonging to another organization.
...
Resource allocation analytics provide task distribution across team members.
...
Configure the required environment variables and database connection in your local configuration.
